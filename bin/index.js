...
commander
    ...
    .option("--list", "List all available assumable roles")
    .parse(process.argv);
...
const listAccounts = commander.list;

Promise.resolve()
    .then(() => {
        if (commander.configure) return configureProfileAsync(profileName);
        return login.loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, listAccounts);
    })
    .catch(err => {
