...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, listAccounts) {
        ...
        if (listAccounts) {
            const roles_array = _.map(roles, "roleArn")
            for (var i = 0; i < roles_array.length; i++)
                console.log(roles_array[i].trim());
        } else {
